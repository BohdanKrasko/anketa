...
exports.create = async (data) => {
    ...
    // if (is_exists) {
    //     await conn.query('DELETE FROM children_answer WHERE children_id = ?', [children_id])
    //             .catch(err => {console.log(err)})
    // }
    for (const key in data.answers) {
        let element = data.answers[key]
            await conn.query('DELETE FROM children_answer WHERE children_id = ? AND question_id = ?', 
                [children_id,element.question_id]).catch(err => {
                    console.log(err)
                    return err;
                })
            await conn.query('INSERT INTO children_answer (children_id, list_of_answer_id, question_id) VALUES (?,?,?)', 
