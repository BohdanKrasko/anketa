...
exports.get = async (data) => {
    ...
    const result = await conn.query("SELECT anketa.anketa_id, anketa.name_of_anketa, section.section_id, section.name_of_section, question.question_id, question.question, list_of_answers.list_of_answers_id, list_of_answers.name_of_answer, children_answer.date, children.name, children.surname, parents.first_name, parents.last_name, parents.phone \
                                        FROM \
                                            anketa \
                                        ...
                                        ORDER BY \
                                            children_answer.date", [data.anketa_id, data.children_id]).then(res => {
                                                return res[0]
                                            }).catch(err => {
                                                console.log(err)
                                                return err
